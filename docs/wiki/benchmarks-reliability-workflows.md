...
> (2026-07-02) or are labelled mechanism-only. The DED clean-floor figures (§7) are synthetic
> self-consistency numbers and are reset-independent; the campaign-1 F1/RMSE **thresholds** (§6)
...
- doc→doc links are relative Markdown **with** the `.md` extension and a stable anchor; doc→code is
  ...
  code→doc is generated by `scripts/build_wiki_codemap.py` — never hand-written.
...
- Regenerate `docs/wiki/code-map.md` whenever frontmatter `code_refs`/`lean_refs` change.
