...
The forward model maps a plasma state `(T, n_e, {C_s})` to a synthetic
spectrum `I(λ)`. The pipeline is:
...
## Inversion: Iterative CF-LIBS Solver

The iterative CF-LIBS solver (`cflibs.inversion.solver.IterativeCFLIBSSolver`)
inverts the forward model. The algorithm is standard Ciucci/Tognoni CF-LIBS
with refinements documented below.
...
[Assumptions and Validity § LTE](Assumptions_And_Validity.md#1-local-thermodynamic-equilibrium-lte).
