...
## Appendix A — Verified leaderboard at n=33 (added 2026-05-13 ~02:50 EDT)

Methodology: same per-identifier `run_unified_benchmark.py` invocation as the smoke (Section 1.1), but **executed independently on 3 disjoint Vrabel shards (1/3, 2/3, 3/3)** in parallel on vasp-01 / 02 / 03. n=11 scored spectra per shard × 3 shards = n=33 per identifier. Each row is the cross-shard mean ± std. Confirmed on-disk via `id_summary.json` for every (identifier, shard) pair.

| Rank | Identifier | F1 mean | F1 std | P mean | R mean | FP/spec mean | Per-shard F1 |
|---|---|---|---|---|---|---|---|
| 1 | **hybrid_union** | **0.7502** | **0.0335** | 0.797 | 0.715 | 1.30 | [0.715, 0.753, 0.782] |
| 2 | spectral_nnls | 0.4633 | 0.0196 | 0.406 | 0.543 | 5.67 | [0.442, 0.468, 0.480] |
| 3 | correlation | 0.1955 | 0.0206 | 0.714 | 0.114 | 0.33 | [0.177, 0.218, 0.191] |
| 4 | alias | 0.1415 | 0.0071 | **1.000** | 0.076 | **0.00** | [0.141, 0.149, 0.135] |
| 5 | comb | 0.0242 | 0.0035 | 0.200 | 0.013 | 0.36 | [0.028, 0.022, 0.023] |

**Key claims that are now defensible at n=33 (no longer "preliminary smoke"):**

1. **hybrid_union is the F1 winner by ~14 standard deviations** over spectral_nnls. The F1 gap (0.75 vs 0.46) is 0.29 absolute; the pooled σ is ~0.027; (0.29 / 0.027) ≈ 14 σ. Rank ordering is statistically airtight at this n.
2. **alias has perfect precision (1.000) and zero false positives (0.00) on every one of the 3 independent shards.** This is the strongest possible cross-shard reproducibility evidence for "strict classifier" behavior.
3. **hybrid_union strictly dominates both its components.** alias F1=0.14, spectral_nnls F1=0.46, hybrid_union F1=0.75. The union recovers ~94 % of spectral_nnls's recall while clipping ~77 % of its false positives (5.67 → 1.30). Cross-shard variability of the union (σ=0.034) is similar to its components (σ=0.020-0.034).

**Honest limitations of n=33:**
- 33 spectra is small by ML-paper standards. Bootstrap CI from n=33 would have a 95 % half-width of roughly ±0.05 on F1.
- All 3 shards drew from the same composite corpus (Aalto LIBS + BHVO-2 USGS + Vrabel 2020). Cross-corpus generalization is *not* tested.
- No composition-workflow data; `--sections id` only.

**What this enables for next-step ADRs (priority adjustment vs. Section 3 above):**
- The identifier ordering is now fixed enough that ADR-0002 / 0003 / 0004 prioritization is grounded in real cross-shard data, not single-iter smoke.
- The ensemble finding (hybrid_union beats components) is strong enough to justify a research bead (`CF-LIBS-improved-r4qz`, paper) at P1 rather than P3.
- The "alias has perfect precision" finding is a CF-LIBS-specific behavioral fact worth surfacing in `cflibs/inversion/identify/alias.py` docstring as canonical behavior, not just a benchmark observation.

## Appendix B — Wrapper bug fix that enabled this analysis

While preparing this analysis, the `parameter_sweep.py` wrapper bug (`CF-LIBS-improved-yfbg`, PR #150) was found and fixed: `--bandit 0` with multi-cell `--cells` was silently running only cell 0 for all iters. Without the fix, no realistic multi-cell sweep could have produced the cross-shard leaderboard above. The data in Appendix A was generated by bypassing the wrapper entirely (raw `run_unified_benchmark.py` invocations in a bash loop, `scripts/exp001_raw_runner.sh`). Now that the wrapper is fixed, future sweeps can use `parameter_sweep.py --bandit 0` for equal-allocation multi-cell sweeps as originally intended.
