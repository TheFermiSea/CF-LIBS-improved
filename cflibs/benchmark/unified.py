...
from cflibs.benchmark.composition_eval import (  # noqa: F401
    # Public surface
...
from cflibs.benchmark.workflows import (  # noqa: F401
    # Feature flags (test_jax_workflows.py imports these from unified)
    HAS_JAX,
    HAS_JAX_ITERATIVE_SOLVER,
    HAS_NUMPYRO,
    IterativeCFLIBSSolverJax,
    SUPPORTED_BOLTZMANN_WEIGHTINGS,
    # Sweep registry constants (test_alias_sweep_workflows_registered.py)
    _ALIAS_SWEEP_BASE_KWARGS,
    _ALIAS_SWEEP_CELLS,
    # Lazy-script-loader infrastructure — used by load_aalto_id_dataset,
    # load_assay_and_blind_datasets, _build_real_benchmark_spectrum below
    _aalto_search_elements,
    _alias_sweep_workflow_configs,
    # Identification predictor builders (re-exported for back-compat)
    ...
    _estimate_effective_rp_lazy,
    _estimate_rp_for_spectrum,
    # Composition fit-predictor builders
    _fit_bayesian_pipeline,
    _fit_hybrid_manifold_pipeline,
    _fit_iterative_jax_pipeline,
    _fit_iterative_pipeline,
    _fit_joint_optimizer_pipeline,
    _jax_identifier_flags_for,
    _load_real_spectra_lazy,
    _load_repo_script_module,
    _pipeline_hybrid_manifold_lazy,
    _pipeline_joint_softmax_lazy,
    _run_boltzmann_pipeline_lazy,
    _select_aalto_cases_lazy,
    _validate_boltzmann_weighting,
)
...
# ---------------------------------------------------------------------------
# Alias-fix sweep harness (Phase C of jaunty-weaving-mist) — definitions live
# in :mod:`cflibs.benchmark.workflows` since 2026-05-19.  ``_ALIAS_SWEEP_CELLS``,
# ``_ALIAS_SWEEP_BASE_KWARGS``, ``_alias_sweep_workflow_configs`` and
# ``_build_alias_sweep_predictor_factory`` are re-imported at module top so
# legacy imports ``from cflibs.benchmark.unified import _ALIAS_SWEEP_CELLS, ...``
# keep working.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Identification predictor builders (``_build_*_predictor`` family) and the
# composition fit-predictor builders (``_fit_*_pipeline`` family) moved to
# :mod:`cflibs.benchmark.workflows` 2026-05-19. They are re-imported at module
# top so existing callers — including
# ``from cflibs.benchmark.unified import _build_alias_predictor`` and the
# parameter-sweep harness in ``scripts/sweep_alias_fixes.py`` — keep working
# verbatim.  See PR refactor/extract-workflow-builders for the rationale.
# ---------------------------------------------------------------------------
...
class UnifiedBenchmarkRunner:
    ...
    def write_outputs(
        ...
        if output_format in {"parquet", "both"} and not results_module.parquet_available():
            ...
            print("[unified] pyarrow not available; falling back to output_format='json' (legacy).")
            output_format = "json"
