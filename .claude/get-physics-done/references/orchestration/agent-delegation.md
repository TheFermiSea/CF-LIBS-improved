...
Task(
  ...
  readonly=false,
  prompt="First, read ./.claude/agents/gpd-{agent}.md for your role and instructions.\n\n{task_prompt}",
...
3. **Agent instructions path:** `./.claude/agents/gpd-{agent}.md` (resolved by installer per runtime)
...
5. **Never hardcode runtime-specific paths** — use `./.claude/get-physics-done` for specs assets and `./.claude/agents` for agent prompts, and let the installer project shell `gpd` calls onto the correct runtime bridge.
...
8. **Assign an explicit write scope for every subagent.** Parallel agents must not share writable files. Prefer `Edit` for targeted changes, and re-read the file immediately before writing.
...
1. **Verify expected artifacts exist on disk** using `Read` or `ls`. Do not trust the subagent's claim that it wrote them.
