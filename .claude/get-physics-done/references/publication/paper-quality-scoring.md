...
|-------|--------|---------------|
| All displayed equations have `\label{}` | 4 | Use `Grep` for unlabeled `\begin{equation}` |
...
| No `MISSING:` placeholder citations remain | 3 | Use `Grep` for `MISSING:` in .tex files |
...
| No UNRELIABLE confidence ratings on any result | 5 | Use `Grep` on VERIFICATION.md for "UNRELIABLE" |
...
| No TODO/FIXME/PENDING placeholders remain | 3 | Use `Grep` in .tex files for placeholders |
...
```bash
/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local --raw validate paper-quality --from-project .
```
...
If you need to provide a manual JSON instead, use `@./.claude/get-physics-done/templates/paper/paper-quality-input-schema.md` as the schema source of truth.
