...
```bash
INIT=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local init phase-op "${target_phase}")
if [ $? -ne 0 ]; then
...
Use Grep to search `.gpd/phases/` for references to the target phase number, its results, or its key outputs.
...
```bash
RESULT=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local phase insert "${target_phase}" "${phase_name_with_revised_prefix}")
if [ $? -ne 0 ]; then
...
Use Grep to search `.gpd/phases/` and `.gpd/ROADMAP.md` for:
...
```bash
/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local state add-decision --phase "${target}" --summary "Superseded Phase ${target} (${name}) with Phase ${replacement_number}" --rationale "${reason}"
```

2. Add a blocker noting the results are under revision:

```bash
/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local state add-blocker --text "Phase ${target} results under revision -- superseded by Phase ${replacement_number}: ${reason}"
```

3. Update last activity:

```bash
/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local state update "Last Activity" "$(date +%Y-%m-%d)"
```
...
```bash
PRE_CHECK=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local pre-commit-check --files .gpd/ROADMAP.md .gpd/STATE.md 2>&1) || true
echo "$PRE_CHECK"

/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local commit "docs(phase-${target_padded}): supersede and replace — ${reason}" --files .gpd/
```
</step>
