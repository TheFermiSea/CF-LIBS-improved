...
```bash
CONTEXT=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local --raw validate command-context arxiv-submission "$ARGUMENTS")
if [ $? -ne 0 ]; then
  echo "$CONTEXT"
  exit 1
fi
...
```bash
if [ -n "$ARGUMENTS" ]; then
  REVIEW_PREFLIGHT=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local validate review-preflight arxiv-submission "$ARGUMENTS" --strict)
else
  REVIEW_PREFLIGHT=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local validate review-preflight arxiv-submission --strict)
fi
if [ $? -ne 0 ]; then
...
```bash
/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local paper-build "${PAPER_DIR}/PAPER-CONFIG.json" --output-dir "${PAPER_DIR}"
```
...
**Quality Score:** {score}/100 ({status}) — see `./.claude/get-physics-done/references/publication/paper-quality-scoring.md`
...
```bash
PRE_CHECK=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local pre-commit-check --files "${SUBMISSION_DIR}/SUBMISSION-MANIFEST.md" "${SUBMISSION_DIR}/00README.XXX" .gitignore 2>&1) || true
echo "$PRE_CHECK"
