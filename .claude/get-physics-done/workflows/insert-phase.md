...
**Reference:** `./.claude/get-physics-done/references/orchestration/agent-infrastructure.md` — See the `Decimal Phase Calculation` section for the numbering rules and troubleshooting details.
</required_reading>
...
```bash
INIT=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local init phase-op "${after_phase}")
if [ $? -ne 0 ]; then
...
```bash
RESULT=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local phase insert "${after_phase}" "${description}")
if [ $? -ne 0 ]; then
...
```bash
/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local state add-decision --phase "${decimal_phase}" --summary "Inserted Phase ${decimal_phase} after Phase ${after_phase}: ${description} (URGENT)" --rationale "Urgent work discovered mid-project requiring immediate attention"
```
