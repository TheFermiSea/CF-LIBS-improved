...
Before deciding any merge or repair, read `./.claude/get-physics-done/templates/state-json-schema.md` itself and use its authoritative-vs-derived rules as the reconciliation contract rather than guessing from the current file contents.
</required_reading>
...
```bash
PRE_CHECK=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local pre-commit-check --files .gpd/STATE.md .gpd/state.json 2>&1) || true
echo "$PRE_CHECK"
