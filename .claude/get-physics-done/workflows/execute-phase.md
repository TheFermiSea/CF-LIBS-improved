...
For agent selection strategy and verification failure routing, see `@./.claude/get-physics-done/references/orchestration/meta-orchestration.md`.
</required_reading>
...
```bash
INIT=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local init execute-phase "${PHASE_ARG}")
if [ $? -ne 0 ]; then
...
```bash
PHASE_GOAL=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local roadmap get-phase "${phase_number}" | /home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local json get .goal --default "")
PLAN_OBJECTIVES=""
for plan in "$phase_dir"/*-PLAN.md; do
  OBJ=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local frontmatter get "$plan" --field objective 2>/dev/null)
...
```bash
CONV_STATUS=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local --raw convention check)
...
for AGENT_TYPE in "${PRE_EXECUTION_AGENTS[@]}"; do
  case "$AGENT_TYPE" in
    notation-coordinator)
      AGENT_MODEL=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local resolve-model gpd-notation-coordinator)
      ...
      ;;
    experiment-designer)
      AGENT_MODEL=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local resolve-model gpd-experiment-designer)
...
```bash
PLAN_INDEX=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local phase index "${phase_number}")
```
...
for WAVE_NUM in $(echo "$PLAN_INDEX" | /home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local json keys .waves); do
  WAVE_PLANS=$(echo "$PLAN_INDEX" | /home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local json list ".waves[\"$WAVE_NUM\"]")
  for PLAN_ID in $WAVE_PLANS; do
    DEPS=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local frontmatter get \
...
for WAVE_NUM in $(echo "$PLAN_INDEX" | /home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local json keys .waves); do
  ...
  if [ ${#WAVE_PLANS[@]} -gt 1 ]; then
    ALL_FILES=()
    for PLAN_ID in "${WAVE_PLANS[@]}"; do
      FILES=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local frontmatter get \
...
MAX_UNATTENDED_MINUTES_PER_WAVE=$(echo "$INIT" | /home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local json get .max_unattended_minutes_per_wave --default 90)
...
4. **Spawn executor agents:**
   ...
   Task(
     ...
     readonly=false,
     prompt="First, read ./.claude/agents/gpd-executor.md for your role and instructions.
       ...
       - Workflow: ./.claude/get-physics-done/workflows/execute-plan.md
       - Summary template: ./.claude/get-physics-done/templates/summary.md
       - Checkpoints ref: ./.claude/get-physics-done/references/orchestration/checkpoints.md
       - Validation ref: ./.claude/get-physics-done/references/verification/core/verification-core.md (+ domain-specific verification file)
...
6. **Report completion -- spot-check claims first:**
   ...
   - Validate the gpd_return envelope:
     ...
     ```bash
     RETURN_CHECK=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local --raw validate-return "${SUMMARY_FILE}")
...
9. **Inter-wave verification gate (if more waves remain):**
   ...
   **If enabled:**
   ...
   wave_summaries=()
   for PLAN_ID in $(echo "$PLAN_INDEX" | /home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local json list ".waves[\"$WAVE_NUM\"]"); do
...
LATER_PLANS=()
for LATER_WAVE in $(echo "$PLAN_INDEX" | /home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local json keys .waves | awk -v w="$WAVE_NUM" '$1 > w'); do
  for P in $(echo "$PLAN_INDEX" | /home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local json list ".waves[\"$LATER_WAVE\"]"); do
    LATER_PLANS+=("$P")
  done
done
...
6. **Spawn continuation Agent (NOT resume)** using `./.claude/get-physics-done/templates/continuation-prompt.md` template:
...
for summary in "${phase_dir}"/*-SUMMARY.md; do
  /home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local summary-extract "$summary" --field one_liner
done
```
</step>
...
```bash
cat ./.claude/get-physics-done/templates/paper/figure-tracker.md
```
...
```bash
PRE_CHECK=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local pre-commit-check --files .gpd/paper/FIGURE_TRACKER.md 2>&1) || true
echo "$PRE_CHECK"

/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local commit \
...
ALL_PLAN_IDS=($(echo "$PLAN_INDEX" | /home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local json pluck .plans id))
...
Create a recovery section in the phase directory. For physics-specific root cause analysis, consult `./.claude/get-physics-done/templates/recovery-plan.md`:
...
```bash
PRE_CHECK=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local pre-commit-check --files "${RECOVERY_FILE}" .gpd/STATE.md 2>&1) || true
echo "$PRE_CHECK"
...
Follow the verification workflow. Read `./.claude/get-physics-done/workflows/verify-phase.md` using the Read tool.
...
```bash
DEBUGGER_MODEL=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local resolve-model gpd-debugger)
```
...
Task(
  ...
  readonly=false,
  prompt="First, read ./.claude/agents/gpd-debugger.md for your role and instructions.
...
```bash
VERIFIER_MODEL=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local resolve-model gpd-verifier)
```
...
Task(
  ...
  readonly=false,
  prompt="First, read ./.claude/agents/gpd-verifier.md for your role and instructions.
...
```bash
CONSISTENCY_MODEL=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local resolve-model gpd-consistency-checker)
```

Spawn the consistency checker in rapid mode:

Task(prompt="First, read ./.claude/agents/gpd-consistency-checker.md for your role and instructions.
...
```bash
NOTATION_MODEL=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local resolve-model gpd-notation-coordinator)
```
...
Task(
  ...
  readonly=false,
  prompt="First, read ./.claude/agents/gpd-notation-coordinator.md for your role and instructions.
...
Follow the full transition protocol. Read `./.claude/get-physics-done/workflows/transition.md` using the Read tool for PROJECT.md evolution, DECISIONS.md updates, and parallel phase detection.

```bash
PRE_CHECK=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local pre-commit-check --files .gpd/ROADMAP.md .gpd/STATE.md "${phase_dir}"/*-VERIFICATION.md .gpd/REQUIREMENTS.md 2>&1) || true
echo "$PRE_CHECK"

/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local commit "docs(phase-${phase_number}): complete phase execution" --files .gpd/ROADMAP.md .gpd/STATE.md "${phase_dir}"/*-VERIFICATION.md .gpd/REQUIREMENTS.md
```
