...
for summary in .gpd/phases/*-*/*-SUMMARY.md; do
  /home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local summary-extract "$summary" --field one_liner | /home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local json get .one_liner --default ""
done
```
...
```bash
DIGEST=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local history-digest)
```
...
/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local --raw convention list
...
```bash
ARCHIVE=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local milestone complete "v[X.Y]" --name "[Milestone Name]")
```
...
/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local --raw pattern list 2>/dev/null
```
...
```bash
/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local --raw pattern search "keyword from insight" 2>/dev/null
```
...
```bash
PRE_CHECK=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local pre-commit-check --files .gpd/milestones/ .gpd/ROADMAP.md 2>&1) || true
echo "$PRE_CHECK"
...
```bash
INIT=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local init milestone-op)
if [ $? -ne 0 ]; then
...
PRE_CHECK=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local pre-commit-check --files $COMMIT_FILES 2>&1) || true
echo "$PRE_CHECK"

/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local commit "chore: complete v[X.Y] research milestone" --files $COMMIT_FILES
```
