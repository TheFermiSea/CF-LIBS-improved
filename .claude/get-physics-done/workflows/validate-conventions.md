...
```bash
CONTEXT=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local --raw validate command-context validate-conventions "$ARGUMENTS")
if [ $? -ne 0 ]; then
  echo "$CONTEXT"
  exit 1
fi
...
```bash
CONVENTIONS=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local convention list)
```
...
for SUMMARY in .gpd/phases/${PHASE_DIR}/*-SUMMARY.md; do
  /home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local summary-extract "$SUMMARY" --field conventions --field affects
done
```
...
Task(
  ...
  readonly=false,
  prompt="First, read ./.claude/agents/gpd-consistency-checker.md for your role and instructions.
