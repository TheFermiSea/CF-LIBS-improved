...
```bash
GPD_INSTALL_DIR="./.claude/get-physics-done"
GPD_CONFIG_DIR="./.claude"
GPD_GLOBAL_CONFIG_DIR="/home/brian/.claude"
GPD_RUNTIME_FLAG="--claude"
INSTALL_SCOPE="--local"
...
> **Platform note:** If `AskUserQuestion` is not available, present the choices in plain text and wait for the user's freeform response.
...
```bash
rm -f \
  "./.claude/cache/gpd-update-check.json" \
  "/home/brian/.claude/cache/gpd-update-check.json" \
