...
- ./.claude/get-physics-done/references/ui/ui-brand.md
- ./.claude/get-physics-done/templates/planner-subagent-prompt.md -- Template for spawning gpd-planner agents (placeholders, continuation format, failure protocol)
- ./.claude/get-physics-done/templates/phase-prompt.md -- PLAN.md output format (frontmatter, task XML, contract-native wiring)
</required_reading>
...
```bash
INIT=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local init plan-phase "$PHASE" --include state,roadmap,requirements,context,research,verification,validation)
if [ $? -ne 0 ]; then
...
PHASE=$(echo "$INIT" | /home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local json get .phase_number --default "${REQUESTED_PHASE}")
PHASE_DIR=$(echo "$INIT" | /home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local json get .phase_dir --default "")
...
RESEARCH_MODE=$(echo "$INIT" | /home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local json get .research_mode --default balanced)
```
...
```bash
PHASE_INFO=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local roadmap get-phase "${PHASE}")
if [ "$(echo "$PHASE_INFO" | gpd json get .found --default false)" != "true" ]; then
  echo "Error: Phase ${PHASE} not found in ROADMAP.md."
  exit 1
fi

PHASE_NAME=$(echo "$PHASE_INFO" | /home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local json get .phase_name --default "")
PHASE_SLUG=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local slug "$PHASE_NAME")
...
```bash
CONV_CHECK=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local --raw convention check 2>/dev/null)
if [ $? -ne 0 ]; then
...
if [ "$RESEARCH_MODE" = "explore" ]; then
  ...
  if echo "$INIT" | /home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local json get .research_content --default "" | grep -qi "method\\|benchmark\\|anchor"; then
...
```bash
PHASE_DESC=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local roadmap get-phase "${PHASE}" | /home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local json get .section --default "")
# Use requirements_content from INIT (already loaded via --include requirements)
REQUIREMENTS=$(echo "$INIT" | /home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local json get .requirements_content --default "" | grep -A100 "## Requirements" | head -50)
STATE_SNAP=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local state snapshot)
# Extract decisions from gpd state snapshot JSON: echo "$STATE_SNAP" | /home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local json list .decisions
```
...
```
Task(
  prompt="First, read ./.claude/agents/gpd-phase-researcher.md for your role and instructions.\n\n" + research_prompt,
...
```bash
PHASE_GOAL=$(echo "$INIT" | /home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local json get .phase_name --default "")
...
```bash
EXPERIMENT_MODEL=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local resolve-model gpd-experiment-designer)
```
...
```
Task(
  prompt="First, read ./.claude/agents/gpd-experiment-designer.md for your role and instructions.\n\n" + experiment_prompt,
...
ROADMAP_CONTENT=$(echo "$INIT" | /home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local json get .roadmap_content --default "")
REQUIREMENTS_CONTENT=$(echo "$INIT" | /home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local json get .requirements_content --default "")
VERIFICATION_CONTENT=$(echo "$INIT" | /home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local json get .verification_content --default "")
UAT_CONTENT=$(echo "$INIT" | /home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local json get .validation_content --default "")
CONTEXT_CONTENT=$(echo "$INIT" | /home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local json get .context_content --default "")
PROJECT_CONTRACT=$(echo "$INIT" | /home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local json get .project_contract --default "")
PROTOCOL_BUNDLE_CONTEXT=$(echo "$INIT" | /home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local json get .protocol_bundle_context --default "")
ACTIVE_REFERENCE_CONTEXT=$(echo "$INIT" | /home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local json get .active_reference_context --default "")
REFERENCE_ARTIFACTS_CONTENT=$(echo "$INIT" | /home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local json get .reference_artifacts_content --default "")
```
...
if [ -n "$RESEARCH_FILE" ]; then
  ...
  RESEARCH_CONTENT=$(echo "$INIT" | /home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local json get .research_content --default "")
fi
```
...
See `./.claude/get-physics-done/references/orchestration/context-budget.md` for detailed budget allocation by workflow type.
</context_budget_guidance>
...
```
Task(
  prompt="First, read ./.claude/agents/gpd-planner.md for your role and instructions.\n\n" + filled_prompt,
...
```
Task(
  prompt="First, read ./.claude/agents/gpd-plan-checker.md for your role and instructions.\n\n" + checker_prompt,
