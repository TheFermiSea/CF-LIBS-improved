...
```bash
INIT=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local init phase-op)
if [ $? -ne 0 ]; then
...
```bash
AUTONOMY=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local --raw config get autonomy 2>/dev/null | /home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local json get .value --default balanced 2>/dev/null || echo "balanced")
RESEARCH_MODE=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local --raw config get research_mode 2>/dev/null | /home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local json get .value --default balanced 2>/dev/null || echo "balanced")
```
...
For detailed mode adaptation specifications (bibliographer search breadth, referee strictness, paper-writer style by mode), see `./.claude/get-physics-done/references/publication/publication-pipeline-modes.md`.

Run centralized context preflight before continuing:

```bash
CONTEXT=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local --raw validate command-context write-paper "$ARGUMENTS")
if [ $? -ne 0 ]; then
  echo "$CONTEXT"
  exit 1
fi
...
```bash
/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local validate review-preflight write-paper --strict
```
...
- Define all custom macros in a preamble block (see `./.claude/get-physics-done/templates/latex-preamble.md` for standard packages, project-specific macros, equation labeling conventions, and SymPy-to-LaTeX integration)
...
```bash
/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local paper-build paper/PAPER-CONFIG.json
```

This emits `paper/main.tex`, writes the artifact manifest, and keeps the manuscript scaffold aligned with the tested `gpd.mcp.paper` package. If no JSON spec exists yet, create `paper/PAPER-CONFIG.json` first using `./.claude/get-physics-done/templates/paper/paper-config-schema.md` as the schema source of truth, and then run `gpd paper-build` before proceeding. The compilation checks in `draft_sections` require `main.tex` to exist.

When authoring `paper/PAPER-CONFIG.json`:

- use the exact top-level fields from `./.claude/get-physics-done/templates/paper/paper-config-schema.md`
...
**Supplemental material:** If the paper requires supplemental material (common for PRL and other letter-format journals), use `./.claude/get-physics-done/templates/paper/supplemental-material.md` for the standard structure (extended derivations, computational details, additional figures, data tables, code availability).

**Experimental comparison:** If the paper compares theoretical predictions with experimental or observational data, use `./.claude/get-physics-done/templates/paper/experimental-comparison.md` for the systematic comparison structure (data source metadata, unit conversion checklist, pull analysis, chi-squared statistics, discrepancy classification with root cause hierarchy).
</step>
...
```bash
WRITER_MODEL=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local resolve-model gpd-paper-writer)
```
...
```
Task(
  prompt="First, read ./.claude/agents/gpd-paper-writer.md for your role and instructions.\n\n" + section_prompt,
...
```bash
BIBLIO_MODEL=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local resolve-model gpd-bibliographer)
```
> **Runtime delegation:** Spawn a subagent for the task below. Adapt the `Task()` call to your runtime's agent spawning mechanism. If `model` resolves to `null` or an empty string, omit it so the runtime uses its default model. Always pass `readonly=false` for file-producing agents. If subagent spawning is unavailable, execute these steps sequentially in the main context.
...
Task(
  ...
  model="{biblio_model}",
  readonly=false,
  prompt="First, read ./.claude/agents/gpd-bibliographer.md for your role and instructions.
...
- `./.claude/get-physics-done/templates/paper/reproducibility-manifest.md`
...
```bash
/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local --raw validate reproducibility-manifest paper/reproducibility-manifest.json --strict
```
...
**7. Run paper quality scoring** (see `./.claude/get-physics-done/references/publication/paper-quality-scoring.md`):
...
```bash
QUALITY=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local --raw validate paper-quality --from-project . 2>/dev/null)
```
