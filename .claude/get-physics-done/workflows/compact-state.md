...
- ./.claude/get-physics-done/templates/state-archive.md -- Template for STATE-ARCHIVE.md (archive structure, what gets archived, archive format)
</required_reading>
...
```bash
RESULT=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local state compact)
if [ $? -ne 0 ]; then
