...
| --------------------------------------------- | ------------------------------------------------------------ |
| Use `Grep` for "limit" to see if limits are mentioned   | Take the limit yourself and compare with known result        |
| Use `Grep` for "dimensions" to see if they are discussed | Assign dimensions to each symbol and verify term consistency |
| Use `Grep` for "convergence" to see if the word appears | Run at 2+ resolutions and measure convergence rate           |
...
<required_reading>
@./.claude/get-physics-done/references/verification/core/verification-core.md
@./.claude/get-physics-done/references/verification/core/verification-numerical.md
@./.claude/get-physics-done/references/protocols/error-propagation-protocol.md
@./.claude/get-physics-done/templates/verification-report.md
</required_reading>
...
```bash
INIT=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local init phase-op "${PHASE_ARG}")
if [ $? -ne 0 ]; then
...
**Verification independence:** Load only what the verifier needs to judge results on their own merits. See @./.claude/get-physics-done/references/verification/meta/verification-independence.md.
...
for plan in "$phase_dir"/*-PLAN.md; do
  /home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local frontmatter get "$plan" --field contract
done
```
...
```bash
for plan in "$phase_dir"/*-PLAN.md; do
  ARTIFACT_RESULT=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local verify artifacts "$plan")
...
```bash
for plan in "$phase_dir"/*-PLAN.md; do
  CHECKS_RESULT=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local verify plan "$plan")
...
5. Reference: @./.claude/get-physics-done/workflows/numerical-convergence.md for detailed methodology
...
Reference: @./.claude/get-physics-done/references/verification/core/verification-core.md (+ relevant domain verification file)
...
See ./.claude/get-physics-done/templates/verification-report.md for complete template.
</step>
