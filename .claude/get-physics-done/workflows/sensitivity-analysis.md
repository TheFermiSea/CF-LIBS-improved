...
If `.gpd/analysis/PARAMETERS.md` exists, use it as the parameter registry (see `./.claude/get-physics-done/templates/parameter-table.md` for the template).
...
```bash
PRE_CHECK=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local pre-commit-check --files "${REPORT_PATH}" .gpd/STATE.md 2>&1) || true
echo "$PRE_CHECK"
