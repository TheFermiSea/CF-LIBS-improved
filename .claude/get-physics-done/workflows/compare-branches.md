...
```bash
PRE_CHECK=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local pre-commit-check --files .gpd/BRANCH-COMPARISON.md 2>&1) || true
echo "$PRE_CHECK"

/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local commit "docs: export branch comparison" --files .gpd/BRANCH-COMPARISON.md
```
