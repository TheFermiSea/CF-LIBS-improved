...
```bash
CONTEXT=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local --raw validate command-context peer-review "$ARGUMENTS")
if [ $? -ne 0 ]; then
  echo "$CONTEXT"
  exit 1
fi
...
```bash
/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local validate review-preflight peer-review --strict
```
...
```bash
READ_MODEL=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local resolve-model gpd-review-reader)
```
...
Task(
  ...
  model="{read_model}",
  readonly=false,
  prompt="First, read ./.claude/agents/gpd-review-reader.md for your role and instructions.
Then read ./.claude/get-physics-done/references/publication/peer-review-panel.md and use its `ClaimIndex` / `StageReviewReport` artifact contract exactly.
...
```bash
LITERATURE_MODEL=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local resolve-model gpd-review-literature)
MATH_MODEL=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local resolve-model gpd-review-math)
```
...
Task(
  ...
  readonly=false,
  prompt="First, read ./.claude/agents/gpd-review-literature.md for your role and instructions.
Then read ./.claude/get-physics-done/references/publication/peer-review-panel.md and use its `StageReviewReport` artifact contract exactly.
...
Task(
  ...
  model="{math_model}",
  readonly=false,
  prompt="First, read ./.claude/agents/gpd-review-math.md for your role and instructions.
Then read ./.claude/get-physics-done/references/publication/peer-review-panel.md and use its `StageReviewReport` artifact contract exactly.
...
```bash
PHYSICS_MODEL=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local resolve-model gpd-review-physics)
```
...
Task(
  ...
  model="{physics_model}",
  readonly=false,
  prompt="First, read ./.claude/agents/gpd-review-physics.md for your role and instructions.
Then read ./.claude/get-physics-done/references/publication/peer-review-panel.md and use its `StageReviewReport` artifact contract exactly.
...
```bash
SIGNIFICANCE_MODEL=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local resolve-model gpd-review-significance)
```
...
Task(
  ...
  readonly=false,
  prompt="First, read ./.claude/agents/gpd-review-significance.md for your role and instructions.
Then read ./.claude/get-physics-done/references/publication/peer-review-panel.md and use its `StageReviewReport` artifact contract exactly.
...
```bash
REFEREE_MODEL=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local resolve-model gpd-referee)
```
...
Task(
  ...
  model="{referee_model}",
  readonly=false,
  prompt="First, read ./.claude/agents/gpd-referee.md for your role and instructions.
Then read ./.claude/get-physics-done/references/publication/peer-review-panel.md, ./.claude/get-physics-done/templates/paper/review-ledger-schema.md, and ./.claude/get-physics-done/templates/paper/referee-decision-schema.md before you write any adjudication artifacts.
