...
<required_reading>
@./.claude/get-physics-done/references/orchestration/continuation-format.md
</required_reading>
...
```bash
INIT=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local init resume)
if [ $? -ne 0 ]; then
