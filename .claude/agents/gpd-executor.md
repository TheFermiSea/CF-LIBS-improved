...
**Tool selection:** For computational tasks, consult `./.claude/get-physics-done/references/tooling/tool-integration.md` for guidance on Python vs Julia vs Mathematica vs Fortran selection, and correct library API usage.

**Reference index:** When starting execution in a new domain or needing guidance on which reference to load, consult `./.claude/get-physics-done/references/execution/executor-index.md` — it maps execution scenarios (QFT, condensed matter, debugging, paper writing, etc.) to the correct reference files.

**State machine:** For valid state transitions during execution (plan states, phase states, milestone lifecycle), see `./.claude/get-physics-done/templates/state-machine.md`.

Load these shared execution contracts before producing runtime-facing artifacts:
@./.claude/get-physics-done/references/tooling/tool-integration.md
@./.claude/get-physics-done/references/execution/executor-index.md
@./.claude/get-physics-done/templates/state-machine.md
@./.claude/get-physics-done/templates/summary.md
@./.claude/get-physics-done/templates/calculation-log.md
...
@./.claude/get-physics-done/references/verification/errors/llm-physics-errors.md
@./.claude/get-physics-done/references/orchestration/agent-infrastructure.md
@./.claude/get-physics-done/references/protocols/order-of-limits.md
...
**Step 4:** If no bundle is selected, or the bundle is clearly incomplete for the task at hand, fall back to `./.claude/get-physics-done/references/execution/executor-index.md` and load only the minimum additional protocols needed from there. If no fallback domain clearly fits, stay with the generic execution flow plus contract-backed anchors and checks instead of forcing the work into a topic bucket.
...
```bash
INIT=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local init execute-phase "${PHASE}")
```
...
```bash
/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local observe event <category> <name> --phase <N> --plan <PLAN> --data '{"key":"value"}' 2>/dev/null || true
```
...
```bash
/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local trace log <event_type> --data '{"description":"<text>"}' 2>/dev/null || true
```
...
**file_read:** `./.claude/get-physics-done/references/execution/executor-subfield-guide.md`

Also consult: `./.claude/get-physics-done/references/physics-subfields.md` for priority checks, red flags, and recommended software per subfield.
...
**Full rules with examples and escalation protocols:** Load `./.claude/get-physics-done/references/execution/executor-deviation-rules.md` on demand.
...
**See @./.claude/get-physics-done/references/orchestration/checkpoints.md**
...
**file_read:** `./.claude/get-physics-done/references/execution/executor-verification-flows.md`
...
**Full protocol with examples:** Load `./.claude/get-physics-done/references/execution/executor-task-checkpoints.md` on demand.
...
**file_read:** `./.claude/get-physics-done/references/execution/executor-completion.md`
...
- Use template: @./.claude/get-physics-done/templates/summary.md
- Include conventions table, key results with confidence tags, deviation documentation
- For multi-step derivation plans: also produce CALCULATION_LOG.md using template at `./.claude/get-physics-done/templates/calculation-log.md`. Record every derivation step, intermediate check, and error caught.
...
**file_read:** `./.claude/get-physics-done/references/execution/executor-worked-example.md`
...
- **Deviation rules (expanded):** `./.claude/get-physics-done/references/execution/executor-deviation-rules.md` — Full rules, examples, and escalation protocols beyond the inline summary
- **Task checkpoints (expanded):** `./.claude/get-physics-done/references/execution/executor-task-checkpoints.md` — Full checkpoint protocol with examples beyond the inline commit type list
- **Approximation selection:** `./.claude/get-physics-done/references/methods/approximation-selection.md` — Decision framework for choosing approximation methods when a task involves non-trivial method selection
- **Physics code testing:** `./.claude/get-physics-done/references/verification/core/code-testing-physics.md` — Patterns for writing tests that catch physics errors (load for TDD tasks)
- **Cross-project patterns:** `./.claude/get-physics-done/references/shared/cross-project-patterns.md` — Pattern library design and lifecycle (runtime integration handled by `consult_cross_project_patterns` step above)
