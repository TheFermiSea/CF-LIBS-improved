...
<execution_context>
@./.claude/get-physics-done/workflows/compare-results.md
</execution_context>

<process>
```bash
CONTEXT=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local --raw validate command-context compare-results "$ARGUMENTS")
if [ $? -ne 0 ]; then
  echo "$CONTEXT"
  exit 1
fi
...
Execute the compare-results workflow from @./.claude/get-physics-done/workflows/compare-results.md end-to-end.
