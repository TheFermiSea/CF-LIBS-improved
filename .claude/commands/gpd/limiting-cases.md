...
<execution_context>
@./.claude/get-physics-done/workflows/limiting-cases.md
</execution_context>
...
**Pre-flight check:**
```bash
CONTEXT=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local --raw validate command-context limiting-cases "$ARGUMENTS")
if [ $? -ne 0 ]; then
  echo "$CONTEXT"
  exit 1
fi
...
Follow the limiting-cases workflow: @./.claude/get-physics-done/workflows/limiting-cases.md
