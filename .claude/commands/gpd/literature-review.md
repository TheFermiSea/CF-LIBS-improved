...
<execution_context>
@./.claude/get-physics-done/workflows/literature-review.md
</execution_context>
...
```bash
CONTEXT=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local --raw validate command-context literature-review "$ARGUMENTS")
if [ $? -ne 0 ]; then
  echo "$CONTEXT"
  exit 1
fi
...
```bash
REVIEWER_MODEL=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local resolve-model gpd-literature-reviewer)
```
...
```
Task(
  prompt="First, read ./.claude/agents/gpd-literature-reviewer.md for your role and instructions.\n\n" + filled_prompt,
