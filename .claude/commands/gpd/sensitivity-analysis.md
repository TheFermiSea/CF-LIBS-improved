...
<execution_context>
@./.claude/get-physics-done/workflows/sensitivity-analysis.md
</execution_context>
...
```bash
CONTEXT=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local --raw validate command-context sensitivity-analysis "$ARGUMENTS")
if [ $? -ne 0 ]; then
  echo "$CONTEXT"
  exit 1
fi
...
Execute the sensitivity-analysis workflow from @./.claude/get-physics-done/workflows/sensitivity-analysis.md end-to-end.
