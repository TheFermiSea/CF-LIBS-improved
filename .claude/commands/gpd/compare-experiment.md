...
<execution_context>
@./.claude/get-physics-done/workflows/compare-experiment.md
</execution_context>
...
**Pre-flight check:**
```bash
CONTEXT=$(/home/brian/.gpd/venv/bin/python -m gpd.runtime_cli --runtime claude-code --config-dir ./.claude --install-scope local --raw validate command-context compare-experiment "$ARGUMENTS")
if [ $? -ne 0 ]; then
  echo "$CONTEXT"
  exit 1
fi
...
Follow the compare-experiment workflow: @./.claude/get-physics-done/workflows/compare-experiment.md
</process>
