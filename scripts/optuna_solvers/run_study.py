...
def _score(db, datasets, n, ov):
    ...
    board = run_scoreboard(AtomicDatabase(db), datasets=datasets, max_spectra=n, seed=7,
                           pipeline_impl="reference", config_overrides=ov)
...
def objective(trial, db, n):
    ...
    if solver == "iterative":
        ov.update(
            ...
            apply_self_absorption=trial.suggest_categorical("apply_self_absorption", ["off", "observable"]),
...
def main():
    ...
    a = ap.parse_args()
    study = optuna.create_study(direction="minimize", study_name=a.study_name, storage=a.storage,
                                load_if_exists=True, sampler=optuna.samplers.TPESampler(seed=7, multivariate=True))
    def cb(st, tr):
        print(f"  trial {tr.number}: {tr.params.get('solver')} rmse={tr.value} (best={st.best_value:.3f})", flush=True)
    ...
    json.dump({"best_dev": study.best_value, "best_config": best, "best_held_out": held,
               "n_trials": len(study.trials)}, open(a.out, "w"), indent=2)
