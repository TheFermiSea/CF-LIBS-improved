...
\begin{frame}{Kernel 2: Vectorized Boltzmann Fitting}
  ...
  \vspace{0.3em}
  \textbf{Fit $y = a + bx$ by weighted least squares} --- minimize
  ...
  \end{equation*}
  with \alert{five scalar accumulators}:
  $S_w\!=\!\sum w_i$,\;
  $S_{wx}\!=\!\sum w_i x_i$,\;
  $S_{wy}\!=\!\sum w_i y_i$,\;
...
\begin{frame}{Five Accumulators $\to$ Closed-Form Solution}
  ...
  \vspace{0.3em}
  \textbf{Uncertainty} from the Gram covariance matrix $(\bm{X}^\top\bm{W}\bm{X})^{-1}$:
  \begin{equation*}
    \sigma_b = \sqrt{S_w / \Delta}, \qquad
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \textbf{Why not QR/SVD?}
      \begin{itemize}
        \item $2{\times}2$ system: $\kappa \sim 10^2$--$10^3$
        \item Loses 2--3 digits $\to$ 12--13 correct in \texttt{float64}
        \item Physical uncertainties $\sim 1$--$10\%$
        \item QR adds $3\times$ cost, zero practical benefit
      \end{itemize}
    \end{column}
    \begin{column}{0.48\textwidth}
      \textbf{Why this matters for GPU:}
      \begin{itemize}
        \item \alert{Branch-free}: same arithmetic path for every element
        \item No warp divergence (no pivoting, no iteration)
        \item Only $10 N_\text{max} + 11$ FLOPs per element
        \item Degenerate fits ($|\Delta| < 10^{-30}$): branchless \texttt{where} $\to$ $T = 0$
      \end{itemize}
    \end{column}
  \end{columns}
\note{
...
\begin{frame}{Pad-and-Mask: Ragged $\to$ Rectangular $\to$ GPU}
  \textbf{Problem:} $D$ elements with \emph{different} line counts (Li: 3, Ca: 12, Fe: 47)

  \vspace{0.3em}
  \textbf{Solution:} Pad all to $N_\text{max}$, mask out padding via weight-zeroing
  \begin{center}
    \begin{tikzpicture}[
      cell/.style={minimum width=0.55cm, minimum height=0.45cm, font=\scriptsize, anchor=center},
      valid/.style={cell, fill=cbBlue!20},
      padded/.style={cell, fill=gray!15, text=gray},
      elabel/.style={font=\scriptsize\bfseries, anchor=east}
    ]
      % Labels
      \node[elabel] at (-0.2, 0)    {Fe};
      \node[elabel] at (-0.2, -0.6) {Ca};
      \node[elabel] at (-0.2, -1.2) {Li};
      % Fe: 5 valid
      \foreach \i in {0,...,4} { \node[valid] at (0.55*\i+0.275, 0) {$w_{\pgfmathparse{int(\i+1)}\pgfmathresult}$}; }
      % Ca: 3 valid + 2 padded
      \foreach \i in {0,...,2} { \node[valid] at (0.55*\i+0.275, -0.6) {$w_{\pgfmathparse{int(\i+1)}\pgfmathresult}$}; }
      \foreach \i in {3,4} { \node[padded] at (0.55*\i+0.275, -0.6) {$0$}; }
      % Li: 2 valid + 3 padded
      \foreach \i in {0,1} { \node[valid] at (0.55*\i+0.275, -1.2) {$w_{\pgfmathparse{int(\i+1)}\pgfmathresult}$}; }
      \foreach \i in {2,3,4} { \node[padded] at (0.55*\i+0.275, -1.2) {$0$}; }
      % Arrow and label
      \draw[->, thick] (3.3, -0.6) -- (4.3, -0.6);
      \node[font=\scriptsize, anchor=west] at (4.4, -0.35) {$\tilde{w} = w \cdot m$};
      \node[font=\scriptsize, anchor=west] at (4.4, -0.85) {Padded $\to$ zero weight};
    \end{tikzpicture}
  \end{center}

  \vspace{0.2em}
  \begin{itemize}
    \item Weight-zeroing makes padding \alert{mathematically invisible}:
          $\tilde{w}=0 \Rightarrow$ contributes $0$ to every accumulator
    \item All arrays shape $(D, N_\text{max})$ $\to$ standard row-wise reductions
    \item \texttt{jnp.sum(w\_masked * x, axis=1)} $\to$ vector of length $D$
    \item Single \texttt{@jit} kernel: no Python loops, no branches, \texttt{float64}
  \end{itemize}

  \vspace{0.2em}
  {\small\textbf{Cost:} $D \times (10\, N_\text{max} + 11)$ FLOPs total.
  10 elements, $N_\text{max}\!=\!50$: \alert{5{,}110 FLOPs} --- one GPU warp.}
\note{
...
% ----------------------------------------------------------
\begin{frame}{Element Identification at Low Resolving Power: No Single Method Suffices}
  \textbf{Problem:} At RP $<$ 1000, every identification method has critical blind spots

  \vspace{0.3em}
  {\small 74 Aalto LIBS spectra (13 pure elements, 61 minerals), RP = 300--1100, 22-element search list}
  ...
  \vspace{0.5em}
  \begin{itemize}
    \item \textbf{NNLS:} Finds almost everything, but flags $\sim$7 false positives/spectrum
    \item \textbf{Hybrid:} Best precision, but misses blended trace elements
    \item \textbf{Key insight:} Algorithms fail on \emph{different} elements $\to$ ensemble opportunity
  \end{itemize}
\note{
...
\begin{frame}{Systematic Identification Benchmark: Five Spectroscopic Methods}
  ...
  \vspace{0.5em}
  \begin{itemize}
    \item Hybrid NNLS+ALIAS: \alert{+17\% F\textsubscript{1}} over ALIAS baseline (best single method)
    \item Voigt deconvolution provides \textbf{no net benefit} at RP $<$ 1000
    \item Mn and Na are spectroscopically intractable across \emph{all} methods
  \end{itemize}
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      ...
      \begin{tikzpicture}[
        ...
        box/.style={draw, rounded corners, minimum width=3.0cm, minimum height=0.55cm},
        arrow/.style={->, thick, cbBlue}
      ]
        \node[box, fill=cbBlue!10] (alg) {5 algorithms\\(NNLS, ALIAS, Voigt, Fwd, Hybrid)};
        \node[box, fill=cbOrange!15, below=0.4cm of alg] (fuse) {Learned fusion\\(per-algorithm weights)};
        \node[box, fill=cbPurple!15, below=0.4cm of fuse] (thresh) {Per-element thresholds\\(22 elements)};
        \node[font=\small\bfseries, below=0.3cm of thresh, cbGreen] (out) {\alert{F\textsubscript{$\beta$}(0.5) = 0.914}};
        \draw[arrow] (alg) -- (fuse);
      ...
      \vspace{0.4em}
      {\small 101 parameters optimized jointly\\
      via CMA-ES (evosax, JAX-native)}
    \end{column}
    \begin{column}{0.49\textwidth}
      \textbf{GPU acceleration enables optimization:}
      \begin{itemize}
        \item 2,048,000 evaluations in \alert{58 seconds}
        \item 45,000 evals/s on a single V100S
        \item \alert{+121\%} F$_\beta$ over hand-tuned baseline
      \end{itemize}

      \vspace{0.3em}
      \textbf{Physically interpretable parameters:}\\[0.2em]
      {\small
      \begin{tabular}{lcc}
        \toprule
      ...
      Mn/Na are spectroscopically intractable\\at RP $<$ 1000}
    \end{column}
  \end{columns}
\note{
...
\begin{frame}{CMA-ES Convergence: Random $\to$ 0.914 in 58 Seconds}
  \begin{columns}[T]
    ...
    \end{column}
    \begin{column}{0.42\textwidth}
      \textbf{Convergence milestones:}
      \begin{itemize}
        \item Gen 0: random init $\to$ 0.497
        \item Gen 1: matches manual best (0.654)
        \item Gen 4: exceeds it (0.662)
        \item Gen $\sim$300: \alert{converged at 0.914}
        \item Gradient magnitude $< 0.003$
      \end{itemize}

      \vspace{0.5em}
      \textbf{Scale:}
      \begin{itemize}
        \item Pop.\ size: 4,096/generation
        \item 500 generations total
        \item \alert{2,048,000 evaluations}
        \item 35,300 evals/s on V100S
      \end{itemize}
    \end{column}
  \end{columns}
\note{
...
\begin{frame}{Optimization Progression: Baseline to GPU Ensemble}
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{block}{Why multi-method consensus wins}
        \begin{itemize}
          \item \textbf{Each method exploits different spectral info}\\
                NNLS: Boltzmann envelope; ALIAS: line positions
          \item System learns \textbf{element-specific criteria}:\\
                NNLS sensitivity for trace, Hybrid specificity for majors
          \item Per-element thresholds reflect \textbf{spectral physics}
        \end{itemize}
      \end{block}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{block}{Computational cost}
        \begin{itemize}
          \item 2,048,000 evaluations
          \item \alert{58 seconds} wall time
          \item 45,000 evals/s on V100S
          \item JAX-native CMA-ES (evosax)
        \end{itemize}
      \end{block}
    \end{column}
  \end{columns}
...
\begin{frame}{Three Contributions}
  ...
  \begin{itemize}
    ...
    \item 2M evaluations in 58 s on V100S --- GPU enables systematic exploration
    \item Physically interpretable: learns spectral physics hierarchy per element
  \end{itemize}

  \vspace{0.5em}
  \centering
  \textbf{GPU acceleration enables real-time inference, systematic benchmarking,\\
