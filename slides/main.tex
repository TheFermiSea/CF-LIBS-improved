...
\begin{frame}[plain]
  \titlepage
\note{
  Good morning. I will present three contributions to GPU-accelerated
  Calibration-Free LIBS. First, a full GPU pipeline with up to 76 times
  speedup and zero accuracy loss. Second, a systematic benchmark showing
  no single element identification algorithm wins at low resolving power.
  Third, a GPU-optimized 101-parameter multi-method consensus system that
  achieves F-beta of 0.914 --- a 121 percent improvement over the baseline
  --- by fusing five algorithms with learned per-element thresholds.
...
% ----------------------------------------------------------
% SLIDE 9: Kernel 2 — Boltzmann fitting: the problem
% ----------------------------------------------------------
...
\note{
  The Boltzmann plot is the workhorse of CF-LIBS temperature extraction.
  For each element, you plot the log of the normalized intensity versus
  the upper-level energy. The slope gives you the temperature.
  The key insight: for a straight-line fit (two parameters), the normal
  equations are a 2-by-2 system that can be solved analytically.
  You never need to construct or invert a matrix --- just compute five
  scalar sums (dot products) and apply Cramer's rule.
  This is the standard result from Bevington and Robinson (2003) and
  Numerical Recipes section 15.2, specialized to two parameters.
}
\end{frame}

% ----------------------------------------------------------
% SLIDE 9b: Boltzmann fitting: the solution
% ----------------------------------------------------------
\begin{frame}{Five Accumulators $\to$ Closed-Form Solution}
  ...
  \end{columns}
\note{
  The closed-form solution reduces the entire Boltzmann fit to five
  dot products, one determinant, and two numerator evaluations.
  For a 2-by-2 system, the condition number is only 100 to 1000,
  losing just 2 to 3 digits in float64. That leaves 12 to 13 correct
  digits --- far more than the 1 to 10 percent physical uncertainties
  in LIBS line intensities justify. QR or SVD would triple the compute
  cost for zero practical accuracy benefit.
  Critically for GPU: the entire computation is branch-free. Every
  element follows the exact same instruction sequence, eliminating
  warp divergence. The only conditional is a branchless where-guard
  for degenerate fits when all upper-level energies are identical.
}
\end{frame}

% ----------------------------------------------------------
% SLIDE 9c: Pad-and-mask batching
% ----------------------------------------------------------
\begin{frame}{Pad-and-Mask: Ragged $\to$ Rectangular $\to$ GPU}
  ...
  \end{columns}
\note{
  The key challenge for GPU Boltzmann fitting is that each element has
  a different number of emission lines --- Li might have 3 lines while
  Fe has 47. GPUs need uniform array shapes.
  We solve this by padding all elements to the maximum line count and
  using a binary mask. The mask is applied by multiplying it into the
  weights: padded entries get weight zero, so they contribute exactly
  nothing to any of the five accumulator sums. The math is identical
  to fitting only the valid lines.
  The entire operation is a single JIT-compiled JAX function. No Python
  loops, no conditional branches. The element dimension D is just a
  batch axis --- all five dot products for all D elements execute in
  parallel. For a typical 10-element LIBS analysis with 50 max lines,
  the total cost is about 5000 floating-point operations --- trivial
  for a GPU that can do trillions per second.
  Float64 is essential: the Gram determinant involves a difference of
  products that can suffer catastrophic cancellation in float32.
}
\end{frame}
...
\note{
  ...
  regardless of how many elements you have --- the five accumulator
  sums for all elements execute in parallel as row-wise reductions.
  CPU time grows linearly because it loops over elements sequentially.
  The slope error of 5.65 times 10 to the minus 14 confirms the
  condition number analysis: we predicted 12 to 13 correct digits
  from the kappa of 100 to 1000 for the 2-by-2 Gram matrix, and
  that is exactly what we observe. This is at machine precision for
  float64, and far exceeds the physical measurement uncertainties.
...
\begin{frame}{Element Identification at Low RP: No Single Method Suffices}
  ...
  \end{itemize}
\note{
  This table shows each algorithm's standalone performance on all 74 Aalto spectra.
  The fundamental problem: no single algorithm achieves both high precision and
  high recall. NNLS finds 92 percent of true elements but with terrible precision.
  The hybrid has the best precision but misses trace elements. Crucially, they
  fail on different elements --- which creates the opportunity for an ensemble.
}
\end{frame}
...
\note{
  This is the systematic benchmark: 93 configurations across five identification
  pathways. The hybrid NNLS+ALIAS achieves the best F1 among individual methods.
  But even the best individual pathway caps at F1 of 0.654. The question is:
  can we do better by combining all five algorithms? This motivates the
  GPU-accelerated ensemble optimization.
}
\end{frame}
...
\begin{frame}{GPU-Optimized Multi-Method Consensus: F\textsubscript{$\beta$}(0.5) = 0.914}
  ...
  \end{columns}
\note{
  The GPU-optimized ensemble feeds all five identification algorithms into
  a learned fusion function with per-element thresholds --- 101 parameters
  total. CMA-ES optimization via evosax evaluated 2 million configurations
  in 58 seconds on a single V100S. The key insight: GPU acceleration
  enables optimization, not just inference. The optimized default threshold
  jumped from 0.30 to 0.90, meaning the baseline was far too permissive.
  NNLS dominates the fusion with weight 2.93. The per-element thresholds
  are physically interpretable: Fe, Si, Al have low thresholds because
  they are reliably identified, while Mn at 0.93 and Na at 0.87 are
  nearly vetoed --- the optimizer independently discovered these are
  intractable false positive sources at low resolving power.
}
\end{frame}
...
\begin{frame}{CMA-ES Convergence: Random $\to$ 0.914 in 58 Seconds}
  ...
  \end{columns}
\note{
  This shows the CMA-ES convergence curve. Starting from random initialization
  at F-beta 0.497, the optimizer matches the best manual configuration by
  generation 1 and exceeds it by generation 4. It converges at 0.914 by
  generation 300, with gradient magnitudes below 0.003 confirming convergence.
  The population size of 4096 per generation means 2 million total evaluations,
  all completed in 58 seconds. This is only possible because GPU acceleration
  enables 35,000 evaluations per second.
}
\end{frame}
...
\begin{frame}{Optimization Progression: Baseline to GPU Ensemble}
  ...
  \end{columns}
\note{
  This slide shows the progression of optimization approaches.
  Manual grid search over 93 configurations found the best F1 of 0.654,
  but this took hours of expert time. GPU CMA-ES over 41 parameters
  of a single algorithm plateaued at F-beta 0.551 --- confirming the
  ceiling for single-algorithm optimization. Expanding to 101 parameters
  across all five algorithms, Sep-CMA-ES reached F-beta 0.914 in just
  58 seconds with 2 million evaluations. The key finding: further
  improvement requires multi-algorithm fusion, not better single-algorithm
  tuning. No amount of threshold optimization on one algorithm matches
  what the ensemble achieves by combining complementary information.
}
\end{frame}
...
\note{
  ...
  GPU FAISS to complete the inference chain. Edge deployment
  on embedded GPUs for field-portable instruments.
  The element identification optimization is largely converged for this
  dataset, so the key next step is validation on additional datasets
  and higher resolving power instruments like the Mechelle 5000.
  We also want to explore transfer learning --- can the optimized
  thresholds generalize to new spectrometer configurations without
  a full 2-million-evaluation re-optimization?
...
\note{
  Three co-equal contributions. First, the GPU-accelerated CF-LIBS pipeline:
  five kernels, up to 76x speedup, zero accuracy loss. Second, a systematic
  benchmark showing no single algorithm wins at low resolving power --- each
  has different strengths. Third, a GPU-optimized 101-parameter ensemble
  that fuses all five algorithms to achieve F-beta 0.914 --- 121 percent
  over baseline --- with 2 million evaluations in 58 seconds. The ensemble's
  parameters are physically interpretable: it independently discovers which
  elements are easy, which are hard, and which are intractable.
  The unifying theme: GPU acceleration enables not just fast inference but
  also the systematic exploration needed to solve element identification.
}
\end{frame}
...
\begin{frame}{Backup: Per-Element Analysis and Optimized Thresholds}
  ...
  \end{columns}
\note{
  This backup slide combines the per-element hybrid analysis with the
  GPU-optimized thresholds. The optimized thresholds are physically
  interpretable: Fe, Si, and Al have thresholds near zero because they
  are reliably identified at any resolving power. Mn at 0.93 and Na
  at 0.87 are nearly vetoed. The optimizer independently discovered
  the element difficulty hierarchy that we identified in the manual
  benchmark analysis. The default threshold tripled from 0.30 to 0.90,
  confirming the baseline was far too permissive. NNLS dominates the
  fusion weights at 2.93, meaning global spectral context is the
  strongest signal for element identification.
}
\end{frame}
