...
from cflibs.inversion.common.element_id import is_element_detected
...
def test_tier2_detection_rules():
    ...
    assert (
        is_element_detected("Fe", score=0.13, n_matched_lines=2, min_score=0.12, min_lines=2)
        is True
    )

    # Mn with same score (should fail because Tier-2 min_score is 0.15)
    assert (
        is_element_detected("Mn", score=0.13, n_matched_lines=2, min_score=0.12, min_lines=2)
        is False
    )

    # Mn with high score but only 1 line (should fail because Tier-2 min_lines is 2)
    assert (
        is_element_detected("Mn", score=0.20, n_matched_lines=1, min_score=0.12, min_lines=1)
        is False
    )

    # Mn with high score and 2 lines (should pass)
    assert (
        is_element_detected("Mn", score=0.20, n_matched_lines=2, min_score=0.12, min_lines=1)
        is True
    )
...
def test_trace_element_fp_suppression(element):
    ...
    assert (
        is_element_detected(element, score=0.13, n_matched_lines=2, min_score=0.12, min_lines=2)
        is False
    )
    # Case: Score is high but only 1 line
    assert (
        is_element_detected(element, score=0.25, n_matched_lines=1, min_score=0.12, min_lines=1)
        is False
    )
