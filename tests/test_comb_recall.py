...
def test_comb_identifier_trace_logic():
    ...
    class MockDB:
        ...
        def get_transitions(self, element, wavelength_min, wavelength_max):
            ...
            return [
                Transition(wavelength_nm=310.0 + i*0.1, ionization_stage=1, A_ki=1e8, g_k=1, E_k_ev=4.0, E_i_ev=0.0)
                for i in range(50)
            ]

    db = MockDB()
    # Initialize with tuned parameters
    identifier = CombIdentifier(db, min_correlation=0.05, min_active_teeth=2)
